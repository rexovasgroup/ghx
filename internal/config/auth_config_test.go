...
func TestBearerAuth(t *testing.T) {
	t.Run("returns false by default", func(t *testing.T) {
		authCfg := newTestAuthConfig(t)
		require.False(t, authCfg.BearerAuth("github.com"))
	})

	t.Run("returns true when globally enabled", func(t *testing.T) {
		authCfg := newTestAuthConfig(t)
		authCfg.cfg.Set([]string{bearerAuthKey}, "enabled")
		require.True(t, authCfg.BearerAuth("github.com"))
	})

	t.Run("returns true when enabled for specific host", func(t *testing.T) {
		authCfg := newTestAuthConfig(t)
		authCfg.cfg.Set([]string{hostsKey, "example.com", bearerAuthKey}, "enabled")
		require.True(t, authCfg.BearerAuth("example.com"))
		require.False(t, authCfg.BearerAuth("github.com"))
	})
	...
	t.Run("host-level setting overrides global", func(t *testing.T) {
		authCfg := newTestAuthConfig(t)
		authCfg.cfg.Set([]string{bearerAuthKey}, "enabled")
		authCfg.cfg.Set([]string{hostsKey, "github.com", bearerAuthKey}, "disabled")
		require.False(t, authCfg.BearerAuth("github.com"))
	})

	t.Run("returns true when GH_BEARER_AUTH env is set", func(t *testing.T) {
		authCfg := newTestAuthConfig(t)
	...
	t.Run("env var overrides config disabled setting", func(t *testing.T) {
		authCfg := newTestAuthConfig(t)
		authCfg.cfg.Set([]string{bearerAuthKey}, "disabled")
		t.Setenv("GH_BEARER_AUTH", "1")
		require.True(t, authCfg.BearerAuth("github.com"))
	})
}
